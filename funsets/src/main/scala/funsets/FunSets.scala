...
object FunSets {
   ...
   */
  def filter(s: Set, p: Int => Boolean): Set = (e: Int) => contains(s, e) && (p(e)==true)
   ...
   * that satisfies `p`.
   */
  def exists(s: Set, p: Int => Boolean): Boolean = !forall(s, x => !p(x) )  // OR operation

  /**
   * Returns a set transformed by applying `f` to each element of `s`.
   */
  def map(s: Set, f: Int => Int): Set = (x : Int) => exists(s, v => (f(v) == x))
