...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase().groupBy(x => x)
      .mapValues(x => x.length).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences match {
      ...
      case head :: tail => {
        ...
        ( for (h <- iterOccurence(head)) yield tails ++ tails.map(l => h::l)).flatten
      }
    }
  }
  def iterOccurence(occurrence: Occurrence): Occurrences =
    occurrence match {
      case (c, 1) => List(occurrence)
      case (c, x) => occurrence::iterOccurence(c, x-1)
    }
    ...
    def _sentenceAnagrams(occurrences: Occurrences) : List[Sentence] = {
      if( occurrences.isEmpty ) List(Nil)
        ...
        for {
          ...
          tail <- _sentenceAnagrams(subtract(occurrences, comb))
        } yield word::tail
      }
    }
